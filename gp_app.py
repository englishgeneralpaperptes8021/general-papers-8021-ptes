...
# --- GOOGLE DRIVE OFFICIAL API LIBRARIES ---
...
def sync_from_drive():
    ...
    try:
        ...
        st.error(f"❌ Authentication Error: Ensure Service Account credentials are in Streamlit Secrets. Details: {e}")
        return
    ...
    for idx, (local_folder, drive_id) in enumerate(FOLDER_IDS.items()):
        ...
        try:
            ...
            st.warning(f"⚠️ Unable to sync `{local_folder}`: {err}")
    ...
    if success_count == total_folders:
        st.success("✅ All folders successfully synced via Google Drive API!")
    else:
...
with st.sidebar:
    ...
    if st.button("🔄 Sync New Files", width="stretch"):
        sync_from_drive()
